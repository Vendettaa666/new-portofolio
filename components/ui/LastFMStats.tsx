...
function SpotifyLogo({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-label="Spotify">
      <path d="M12 2C6.477 2 2 6.477 2 12s4.477 10 10 10 10-4.477 10-10S17.523 2 12 2zm4.586 14.424c-.18.295-.565.387-.86.207-2.377-1.454-5.37-1.783-8.894-.982-.336.074-.67-.14-.744-.477-.074-.336.14-.67.477-.743 3.856-.88 7.15-.505 9.814 1.128.295.18.387.563.207.857zm1.225-2.72c-.227.367-.707.487-1.074.26-2.72-1.672-6.87-2.157-10.078-1.182-.413.125-.847-.107-.972-.52-.125-.413.107-.847.52-.972 3.67-1.114 8.24-.57 11.35 1.344.366.226.486.706.26 1.073zm.105-2.836C14.392 8.71 8.422 8.51 4.975 9.557c-.55.166-1.124-.15-1.29-.7-.167-.55.15-1.123.7-1.29 3.963-1.202 10.556-.975 14.62 1.437.496.296.658.934.362 1.43-.296.497-.934.658-1.43.363z"/>
    </svg>
  );
}
...
function Skeleton({ className }: { className?: string }) {
  return (
    <div
      className={`rounded-lg bg-neutral-100 dark:bg-neutral-700/60 animate-pulse ${className}`}
    />
  );
}
...
function ArtFallback({
  ...
  return (
    <div
      className={`${size} ${rounded} bg-neutral-200 dark:bg-neutral-700 flex items-center justify-center shrink-0`}
    >
...
function ArtistAvatar({
  ...
  useEffect(() => {
    ...
    if (serverImage) {
      setPhoto(serverImage);
      return;
    }
    // Return cached result if we already fetched this artist
    if (itunesCache.has(name)) {
      setPhoto(itunesCache.get(name) ?? null);
      return;
    }
...
function convertScrobblesToTime(n?: number | string) {
  const num = typeof n === 'string' ? parseInt(n, 10) : (n ?? 0);
  if (isNaN(num) || num <= 0) return { hours: '0', days: '0', isMoreThanADay: false };
  
  const totalMinutes = num * 3.5;
  const totalHours = totalMinutes / 60;
  const totalDays = totalHours / 24;

  return {
    hours: totalHours >= 10 ? Math.round(totalHours).toLocaleString('id-ID') : totalHours.toFixed(1),
    days: totalDays >= 10 ? Math.round(totalDays).toLocaleString('id-ID') : totalDays.toFixed(1),
    isMoreThanADay: totalHours >= 24
  };
}
// ─── Main Component ───────────────────────────────────────────────────────────
export default function LastFMStats() {
  ...
  const maxArtistPlay = Math.max(
    1,
    ...topArtists.map((a) => toNum(a.playcount)),
  );
      ...
      Listening Activity
          ...
          {loading ? (
            ...
            (() => {
              ...
              const artUrl =
                (track as NowPlayingData).albumArt ??
                (track as Track).image ??
                "";
                ...
                >
                  ...
                  <div className="relative shrink-0 h-16 w-16">
                    {artUrl ? (
                      ...
                      <ArtFallback
                        size="h-16 w-16"
                        rounded={isPlaying ? "rounded-full" : "rounded-xl"}
                      />
                    )}
                    {isPlaying && (
                  ...
                  {/* Arrow */}
                  <svg
                    className="h-4 w-4 shrink-0 text-neutral-300 dark:text-neutral-600 group-hover:text-primary group-hover:translate-x-0.5 transition-all duration-150"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth="2"
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {loading &&
                [...Array(6)].map((_, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-3 px-1 py-2 animate-pulse"
                  >
              ...
              {/* Track rows */}
              {!loading &&
                filteredRecent.slice(0, 6).map((track, i) => (
                  <a
                  ...
                  >
                    ...
                    {track.image ? (
                      <img
                        src={track.image}
                        alt={track.title}
                        className="h-9 w-9 shrink-0 rounded-lg object-cover shadow-sm group-hover:scale-105 group-hover:shadow-md transition-all duration-200"
                      />
                    ) : (
                    ...
                    <svg
                      className="h-3.5 w-3.5 shrink-0 text-neutral-200 dark:text-neutral-700 group-hover:text-primary group-hover:translate-x-0.5 transition-all duration-150"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth="2.5"
                  ...
                  >
                    ...
                    {track.image ? (
                      <img
                        src={track.image}
                        alt={track.title}
                        className="h-10 w-10 shrink-0 rounded-lg object-cover shadow-sm group-hover:scale-105 transition-transform duration-200"
                      />
                    ) : (
              ...
              {loading && (
                <div className="flex flex-wrap gap-2">
                  {[80, 120, 60, 100, 70, 90, 50, 110, 75, 95].map((w, i) => (
                    <div
                      key={i}
                      className="h-7 rounded-full bg-neutral-100 dark:bg-neutral-700/60 animate-pulse"
                      style={{ width: w }}
                    />
                  ))}
                </div>
              ...
              {userInfo && (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {/* Total scrobbles & Waktu Mendengarkan */}
                    <div className="col-span-2 grid grid-cols-2 gap-3">
                      <div className="flex flex-col gap-1 p-3 rounded-xl bg-neutral-50 dark:bg-neutral-800/50 border border-neutral-100 dark:border-neutral-700/50">
                        ...
                        <p className="text-xl font-bold text-neutral-900 dark:text-white">
                          {toNum(userInfo.totalScrobbles).toLocaleString(
                            "id-ID",
                          )}
                        </p>
                      </div>

                      {/* Konversi Waktu Terpakai */}
<div className="flex flex-col gap-1 p-3 rounded-xl bg-neutral-50 dark:bg-neutral-800/50 border border-neutral-100 dark:border-neutral-700/50 relative group/time">
  <div className="flex items-center gap-1.5">
    <Radio className="h-3.5 w-3.5 text-primary shrink-0" />
    <span className="text-[10px] uppercase tracking-widest font-semibold text-neutral-400 dark:text-neutral-500 cursor-help  border-neutral-300 dark:border-neutral-600">
      Waktu Putar
    </span>
    {/* Tooltip Keterangan */}
    <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 bg-neutral-900 text-white text-[10px] rounded-lg opacity-0 pointer-events-none group-hover/time:opacity-100 transition-opacity duration-200 z-10 text-center shadow-lg font-normal normal-case">
      Estimasi berbasis rata-rata durasi 3.5 menit per lagu.
    </span>
  </div>
  
  {(() => {
    const time = convertScrobblesToTime(userInfo.totalScrobbles);
    return (
      <p className="text-base font-bold text-neutral-900 dark:text-white mt-0.5 tracking-tight flex flex-wrap items-baseline gap-x-1.5">
        <span>~{time.hours} Jam</span>
        {time.isMoreThanADay && (
          ...
          <div className="flex items-center justify-between pt-1 border-t border-neutral-100 dark:border-neutral-700/50">
            <div className="flex items-center gap-1.5 text-[11px] text-neutral-400 dark:text-neutral-500">
              ...
              Informasi ini tersinkronisasi secara otomatis dengan aktivitas
              saya saat ini secara real-time.
            </div>
            <a
            href={
              userInfo
                ? `https://www.last.fm/user/${userInfo.name}`
                : "https://www.last.fm"
            }
            target="_blank"
            rel="noopener noreferrer"
